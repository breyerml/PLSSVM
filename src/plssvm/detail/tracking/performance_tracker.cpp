...
void performance_tracker::add_tracking_entry(const tracking_entry<plssvm::parameter> &entry) {
    ...
    if (this->is_tracking()) {
        ...
        tracking_entries_["parameter"].emplace("gamma", std::vector<std::string>{ fmt::format("{}", entry.entry_value.gamma) });
...
void performance_tracker::add_tracking_entry(const tracking_entry<cmd::parser_train> &entry) {
    ...
    if (this->is_tracking()) {
        ...
        tracking_entries_[entry.entry_category].emplace("gamma", std::vector<std::string>{ fmt::format("{}", entry.entry_value.csvm_params.gamma) });
